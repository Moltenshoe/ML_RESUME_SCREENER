...
model = LinearSVC(class_weight="balanced", C=1.2, max_iter=5000)
...
with open(os.path.join(output_dir, "classification_report.txt"), "w") as f:
    f.write(f"Model 2 Accuracy: {accuracy}\n\n")
    f.write(classification_report(y_test, y_pred))

with open(os.path.join(output_dir, "metrics.json"), "w") as f:
    json.dump(
        {"accuracy": accuracy, "classification_report": report, "model_version": "v2"},
        f,
        indent=4,
    )
...
print(f"\nModel 2 training artifacts saved in: {output_dir}")